...
void Display3() {
  ...
  glBegin(GL_QUADS);  {
    ...
    // !!! It wasn't filling because the vertices we're written clockwise
...
void Display4() {
  ...
  glBegin(GL_TRIANGLES); {
    // !!! vertices are written clockwise, shape won't be filled
    glColor3f(0.9, 0.1, 0.1);
    glVertex2f(0.9, 0.9);
    glVertex2f(0.2, 0.9);
    glVertex2f(0.9, 0.2);

    // !!! vertices are written COUNTER-CLOCKWISE, shape is filled
    glColor3f(0.1, 0.1, 0.9);
    glVertex2f(0.1, 0.1);
    glVertex2f(0.1, 0.8);
    glVertex2f(0.8, 0.1);
  ...
  */
  ...
  rootY = -0.45;
  radius = 0.45;
  // !!! modified maxSteps so it's more obviously a polygon...
  maxSteps = 10;
  ...
  glBegin(GL_POLYGON); {
    for(int ii = 0; ii < maxSteps; ++ii) {
      ...
      // !!! removing the '-' makes the circle not be filled in because it reverses
      // !!! the direction the vertices are placed in
